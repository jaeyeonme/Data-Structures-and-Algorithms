...
def log_all_pairs(array):
    ...
    # There are nested for loops in this function but there is only one variable array. So we don't neet two variables for the Big-O

    for i in range(len(array)):         # n*0(n)
        for j in range(len(aaray)):     # n*0(n)
            print(array[i], array[j])   # n*n*0(1)
...
def print_numbers_then_pairs(array):
    ...
    # There are a total of three loops here but only variable. So we need only variable for our Big-O notation
