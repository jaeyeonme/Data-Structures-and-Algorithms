...
class my_array():
    ...
    # Now, when the instance of the class is printed, it returns of the array as output
    ...
    # To suit our needs.
    def __str__(self):
        # This will print the attributes of the array class (length and dsata) in string format when print(array_instance) is executed
    ...
    def pop(self):
        last_item = self.data[self.length - 1]  # Coolects the last element
        # Deletes the last element from the array
        del self.data[self.length - 1]
        self.length -= 1  # Decrements the length attribute of the array by 1
        return last_item  # Returns the poped elemetn. O(1) time
    ...
    def delete(self, index):
        for i in range(index, self.length - 1):
            # Shifts elements from the given index to the end by one place towrds left
            self.data[i] = self.data[i + 1]
        # The last element which remains two times in the array is deleted
        del self.data[self.length - 1]
        self.length -= 1  # The length is decremented by 1. O(n) operation
